...
sp = zeros(size(theimg));
 while(levels ~= 0) % While count not equal to 0
    ...
    rowsMatrix(:,1:floor(size(m1,2)/2))=m1(:,1:2:size(m1,2)-1);
    
    %Downsampling of highpass filter on columns:
    rowsMatrix(:,floor(size(m2,2)/2)+1:end)=m2(:,1:2:size(m2,2));
    ...
    sp(1:floor(size(m1,2)/2),1:size(m1,2))=m1(1:floor(size(m1,2)/2),:);
    
    %Downsampling of highpass filter:
    sp(floor(size(m2,2)/2)+1:size(m2,2),1:size(m2,2))=m2(1:floor(size(m2,2)/2)+1,:);
    
     % Extract approximation image:
     theimg = sp(1:floor(size(theimg,1)/2),1:floor(size(theimg,2)/2));
