function wind_speed = qxrd_bz_bb_beta3(type, index, P, f, weights, wws, dds)
% type = 1;
weight = 1e8;
...
if f == 1
    ...
    wind_speed = (co_a*exp(co_b*wind_speed_coeff) + co_c*exp(co_d*wind_speed_coeff))/weight + indd(index) + (rand-0.5)*indd(index)/150;
    if wws ~= 3.5
...
elseif f == 2 % GLCM论文
    ...
    co_b = 1.7286;
    ind_temp = indd(index);
    wind_speed = GLCM_ws(co_a, co_b, P, weights{1}, ind_temp);
elseif f == 3 % GLCM专利
    co_a = -76;
    co_b = 14;
    ind_temp = indd(index);
